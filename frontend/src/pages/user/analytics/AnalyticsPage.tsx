...
const AnalyticsPage = () => {
  ...
  const fetchTrendData = async () => {
    ...
    try {
      ...
      setAiError('Advanced insights are temporarily unavailable.')
    } finally {
      setLoadingAI(false)
    }
  }
    ...
    >
      <main className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-4 flex flex-wrap gap-2">
          {(['week', 'month', 'year'] as TimeRange[]).map(range => (
            <Button
              ...
              className={
                timeRange === range
                  ? 'bg-primary text-white hover:bg-primary/90 px-4 py-1.5'
                  : 'border-primary text-primary hover:bg-primary/10 px-4 py-1.5'
              }
            >
        ...
        <Tabs defaultValue="mood" className="space-y-8">
          <TabsList className="grid w-full grid-cols-4 p-1 bg-secondary/50 rounded-lg">
            <TabsTrigger
              value="mood"
              ...
              <div className="col-span-2 p-6 bg-blue-50 border border-blue-200 rounded-xl text-blue-800">
                ...
                <p className="text-sm">
                  {trendData.insights[0] ||
                    'Please continue logging your mood for at least 7 days to enable trend analysis.'}
                </p>
              </div>
        ...
        {showExportModal && <ExportPreviewModal />}
      </main>
    </DashboardLayout>
  )
}
